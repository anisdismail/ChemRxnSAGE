...
from models.LSTM_LM import LSTM_LM
...
class LSTMLMTrainer:
    def __init__(self, config):
        ...
        with open(os.path.join(config["data_dir"], "centroids_200.data"), "r", encoding='utf-8') as f:
            self.centroids = np.loadtxt(f)
        with open(os.path.join(config["data_dir"], "centroids_strings_200.data"), "r", encoding='utf-8') as f:
...
class VAETrainer:
    def __init__(self, config):
        ...
        self.encoder = LSTMEncoder(
            self.config, vocab_size, model_init, emb_init)
        self.decoder = LSTMDecoder(self.config, vocab, model_init, emb_init)
    ...
    def eval_nll(self,  data_iter):
        ...
        for data, target in data_iter:
            ...
            loss, loss_rc, loss_kl = self.vae.loss(
                data, target, 1.0, nsamples=self.config.nsamples)
    ...
    def train(self):
        ...
        for epoch in range(self.config.epochs):
            ...
            for data, target in self.train_iter:
                ...
                loss, loss_rc, loss_kl = self.vae.loss(
                    data, kl_weight, nsamples=self.config.nsamples)
                loss = loss.mean(dim=-1)
                loss.backward()
                torch.nn.utils.clip_grad_norm_(
                    self.vae.parameters(), self.config.clip_grad)
                ...
                print(
                    f'epoch: {epoch}, avg_loss: {train_loss:.4f}, kl: {report_kl_loss / report_num_sents:.4f}, recon: {report_rec_loss / report_num_sents:.4f}')
            ...
            if loss < best_loss:
                ...
                print(
                    f'update best loss: {best_loss}, best_nll: {best_nll}, best_kl: {best_kl}, best_pll: {best_ppl}')
            ...
            if loss > opt_dict["best_loss"]:
                ...
                if opt_dict["not_improved"] >= self.config.decay_epoch and epoch >= 15:
                    ...
                    decay_cnt += 1
                    self.vae.load_state_dict(torch.load(self.config.save_path))
